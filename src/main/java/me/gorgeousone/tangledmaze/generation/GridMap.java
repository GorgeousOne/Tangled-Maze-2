...
public class GridMap {
	...
	public void setEntrance(Vec2 entranceLoc, Direction facing) {
		...
		System.out.println("path start " + entranceGridPos);
	}
	...
	public void setExit(Vec2 exitBlockLoc, Direction facing) {
		...
		exit.extend(wallWidth);
		exits.add(exit);
		
		Vec2 exitEnd = exit.getEnd();
		
		Vec2 exitGridPos = getGridPos(exitEnd);
		System.out.println(getGridPos(exit.getStart()));
		setPathType(exitGridPos, PathType.EXIT);
		pathStarts.add(getCell(exitGridPos));
		System.out.println("path start " + exitGridPos);
	}
